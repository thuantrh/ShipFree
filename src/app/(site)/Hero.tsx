...
const HeroSection = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 mt-6 min-h-screen flex items-center px-4 sm:px-6 md:px-8 lg:px-12 relative overflow-hidden">
      {/* Animated background elements - Chỉ render ở client side */}
      {isClientSide && (
        <div className="absolute inset-0 overflow-hidden">
          {[...Array(20)].map((_, i) => {
            // Sử dụng deterministic values dựa trên index  
            const top = 5 + (i * 4.75) % 100;
            const left = 10 + (i * 4.25) % 100;
            const size = 50 + (i * 10) % 200;
